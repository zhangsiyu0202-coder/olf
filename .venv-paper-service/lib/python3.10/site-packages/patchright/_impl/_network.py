...
if TYPE_CHECKING:
    ...
    from patchright._impl._page import Page, Worker
...
class Request(ChannelOwner):
    ...
    async def sizes(self) -> RequestSizes:
        ...
        return await response._channel.send("sizes", None)
    ...
    async def response(self) -> Optional["Response"]:
        return from_nullable_channel(await self._channel.send("response", None))
    ...
    async def abort(self, errorCode: str = None) -> None:
        await self._handle_route(
            lambda: self._race_with_page_close(
                self._channel.send("abort", None, {"errorCode": errorCode})
            )
        )
    ...
    def close(self, code: int = None, reason: str = None) -> None:
        _create_task_and_ignore_exception(
            ...
            self._ws._channel.send(
                "closeServer", None, {"code": code, "reason": reason, "wasClean": True}
            ),
        )
...
class WebSocketRoute(ChannelOwner):
    ...
    def connect_to_server(self) -> "WebSocketRoute":
        ...
        asyncio.create_task(self._channel.send("connect", None))
    ...
    async def _after_handle(self) -> None:
        if self._connected:
            return
        try:
            await self._channel.send("ensureOpened", None)
        except Exception:
            pass
...
class Response(ChannelOwner):
    ...
    async def _actual_headers(self) -> "RawHeaders":
        if not self._raw_headers_future:
            ...
            headers = cast(
                HeadersArray, await self._channel.send("rawResponseHeaders", None)
            )
    ...
    async def server_addr(self) -> Optional[RemoteAddr]:
        return await self._channel.send("serverAddr", None)

    async def security_details(self) -> Optional[SecurityDetails]:
        return await self._channel.send("securityDetails", None)
    ...
    async def body(self) -> bytes:
        binary = await self._channel.send("body", None)
