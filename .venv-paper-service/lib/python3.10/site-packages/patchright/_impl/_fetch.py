...
class APIRequestContext(ChannelOwner):
    ...
    async def fetch(
        ...
        assert request or isinstance(urlOrRequest, str), (
    ...
    async def _inner_fetch(
        ...
        assert (1 if data else 0) + (1 if form else 0) + (1 if multipart else 0) <= 1, (
            "Only one of 'data', 'form' or 'multipart' can be specified"
        )
        assert maxRedirects is None or maxRedirects >= 0, (
            "'max_redirects' must be greater than or equal to '0'"
        )
        assert maxRetries is None or maxRetries >= 0, (
